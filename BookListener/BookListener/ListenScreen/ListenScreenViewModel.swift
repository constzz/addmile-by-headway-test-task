...
final class ListenScreenViewModel: ListenScreenViewModelProtocol {
    ...
    init(
        ...
        changePlaybackSpeedSubject.sink { [weak self] _ in
            ...
            var newValue = currentSpeedSubject.value + 0.25
            if newValue > 2.0 {
                newValue = 0.25
            }
            self.audioViewModel.speed = Float(newValue)
            currentSpeedSubject.send(newValue)
        ...
        sliderChangeSubject
            ...
            .sink { [weak self] _ in
                guard let self else { return }
                isEditingCurrentTimeSubject.send(true)
                if isPlayingNonUpdatingValue {
                    togglePlayPause()
                    wasPlayingBeforeSliderChange.send(true)
                }
            }.store(in: &cancellables)
        ...
        currentTimeInSeconds
            ...
            .sink { [weak self] currentTimeInSecond, sliderChange in
                guard let self else { return }
                let isEditingCurrentTime = isEditingCurrentTimeSubject.value
                progressSubject.send(isEditingCurrentTime ? sliderChange : (currentTimeInSecond / totalDuration) * 100)
                currentTimeSubject.send(isEditingCurrentTime
